...
/* STENCIL START */ 
    ...
    //   additionally specify parent and child joints for each link
        var child = robot.joints[x].child;
        robot.links[child].parent = x;
        var parent = robot.joints[x].parent;
        if (typeof robot.links[parent].children === 'undefined')
            robot.links[parent].children = [];
        robot.links[parent].children.push(x);
